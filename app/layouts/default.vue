    ...
    <header class="sticky top-0 z-50 border-b border-gray-200/50 bg-white/95 backdrop-blur-md">
      <nav class="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        ...
        <a href="/" class="text-xl font-bold text-gray-900 transition-colors hover:text-primary">
          Jimmy<span class="text-primary">.</span>
        </a>
      ...
      <div class="mx-auto max-w-7xl px-6 py-12">
        <div class="flex flex-col items-center justify-between gap-6 md:flex-row">
          ...
          <div class="flex items-center gap-4">
            <a 
              ...
              class="rounded-full bg-gray-100 p-3 text-gray-600 transition-all hover:bg-primary hover:text-white"
              aria-label="GitHub"
            >
        ...
        <div class="mt-8 border-t border-gray-100 pt-8 text-center">
          <p class="text-sm text-gray-500">
            © {{ new Date().getFullYear() }} Jimmy. {{ t('footer.rights') }}
          </p>
        </div>
      </div>
    </footer>
...
// Get available locales for switcher with proper typing
const availableLocales = computed<LocaleObject[]>(() => {
  return (locales.value as LocaleObject[]).filter(
    (loc: LocaleObject) => typeof loc !== 'string'
  )
})
</script>
